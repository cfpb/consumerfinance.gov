...
until psql "postgresql://${PGUSER}:${PGPASSWORD}@postgres/${PGDATABASE}" -c '\q' >/dev/null 2>&1; do
...
if [ "$RUN_MIGRATIONS" = true ]; then
  if ! psql "postgresql://${PGUSER}:${PGPASSWORD}@postgres/${PGDATABASE}" -c 'SELECT COUNT(*) FROM auth_user' >/dev/null 2>&1; then
