...
describe( 'The Filing Instruction Guide search functionality', () => {
  describe( 'The search data generator', () => {
    ...
    it( 'should build a list of search items', () => {
      ...
      expect( search.getSearchData( sections )[0].title ).toEqual( '1. What is the FIG?' );
      expect( search.getSearchData( sections )[0].contents ).toContain( '1. What is the FIG?' );
      expect( search.getSearchData( sections )[0].contents ).toContain( 'Lorem Ipsum' );

      expect( search.getSearchData( sections )[10].title ).toEqual( 'Field 3: Application Method' );
      expect( search.getSearchData( sections )[10].contents ).toContain( 'Field 3: Application Method' );
      expect( search.getSearchData( sections )[10].contents ).toContain( 'means by which the applicant submitted the application' );
    } );
