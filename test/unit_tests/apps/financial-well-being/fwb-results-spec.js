...
const selectedClass = 'comparison-chart_toggle-button__selected';
...
describe( 'fwb-results', () => {
  ...
  it( 'should show the initial category on page load', () => {
    initFwbResults();
    expect(
      toggleButtons[0].classList.contains( selectedClass )
    ).to.equal( true );
    expect( dataPoint[0].classList.contains( hiddenClass ) ).to.equal( false );
    expect( dataPoint[1].classList.contains( hiddenClass ) ).to.equal( false );
  } );

  it( 'should hide the other categories on page load', () => {
    initFwbResults();
    expect(
      toggleButtons[1].classList.contains( selectedClass )
    ).to.equal( false );
    expect( dataPoint[4].classList.contains( hiddenClass ) ).to.equal( true );
    expect( dataPoint[5].classList.contains( hiddenClass ) ).to.equal( true );
  } );
  ...
  it( 'should hide the initial category content ' +
    ...
    expect( dataPoint[0].classList.contains( hiddenClass ) ).to.equal( true );
    expect( dataPoint[1].classList.contains( hiddenClass ) ).to.equal( true );
  } );
  ...
  it( 'should show the correct category content ' +
    ...
    expect( dataPoint[4].classList.contains( hiddenClass ) ).to.equal( false );
    expect( dataPoint[5].classList.contains( hiddenClass ) ).to.equal( false );
  } );
} );
