            ...
            steps {
                script {
                    timeout(time: 15, unit: 'MINUTES') {
                        ...
                        sh "docker run -v cypress:/app/cypress -v cypress.json:/app/cypress -w /app -e CYPRESS_baseUrl=https://${CFGOV_HOSTNAME} -e CI=1 cypress/included:4.10.0 npx cypress run -b chrome --headless"
                    }
                }
            }
        }
