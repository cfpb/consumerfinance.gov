...
from jobmanager.models import (
    ApplicantType, Grade, JobCategory, JobLength, MajorCity, Office, Region,
    ServiceType, State
)
...
class GradeTests(TestCase):
    def test_str(self):
        self.assertEqual(
            str(Grade(grade='53', salary_min=1, salary_max=100)),
            '53'
        )
...
class StateTests(TestCase):
    def test_str(self):
        self.assertEqual(
            str(State(name='foo', abbreviation='fo', region_id='ba')),
            'foo'
        )
