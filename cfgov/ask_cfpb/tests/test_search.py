...
class TestAnswerPageSearch(ElasticsearchTestsMixin, TestCase):
    ...
    def test_AnswerPageSearch_autocomplete(self):
        test_answer_page = AnswerPage(title="Money 101", question="What is money?", slug="test-answer-page", live=True)
        ...
        self.assertEqual(test_answer_page_search.autocomplete()[0]['question'], "What is money?")

    def test_AnswerPageSearch_autocomplete_error(self):
        with mock.patch("ask_cfpb.documents.AnswerPageDocument.search", side_effect=RequestError):
            results = AnswerPageSearch.autocomplete('mone')
            self.assertEqual(results, [])
    
    def test_AnswerPage_search(self):
        test_answer_page = AnswerPage(title="Money 101", question="What is money?", answer_content="Money makes the world go round.", slug="test-answer-page", live=True)
        self.ROOT_PAGE.add_child(instance=test_answer_page)
        self.rebuild_elasticsearch_index("ask_cfpb", stdout=StringIO())
        search_term = "What is money?"
        test_answer_page_search = AnswerPageSearch(search_term=search_term)
        test_answer_page_search_results = test_answer_page_search.search()['results'][0].text.rstrip()
        self.assertEqual(test_answer_page_search_results, "What is money?")

    def test_AnswerPage_suggest(self):
        test_answer_page = AnswerPage(title="Money 101", question="What is money?", answer_content="Money makes the world go round.", slug="test-answer-page3", live=True)
        self.ROOT_PAGE.add_child(instance=test_answer_page)
        self.rebuild_elasticsearch_index("ask_cfpb", stdout=StringIO())
        search_term = "monye"
        test_answer_page_search = AnswerPageSearch(search_term=search_term)
        test_answer_page_search_results = test_answer_page_search.suggest()
        self.assertEqual(test_answer_page_search_results['search_term'], "money")

    def test_AnswerPage_suggest_error(self):
        with mock.patch("ask_cfpb.documents.AnswerPageDocument.search", side_effect=IndexError):
            try:
