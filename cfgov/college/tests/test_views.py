...
class TestViews(django.test.TestCase):
    ...
    def test_feedback(self):
        response = self.client.get(reverse('college:disclosures:pfc-feedback'))
        self.assertEqual(
...
class SchoolSearchTest(django.test.TestCase):
    ...
    @mock.patch('college.views.SearchQuerySet.autocomplete')
    def test_school_search_api(self, mock_sqs_autocomplete):
        ...
        url = "%s?q=Kansas" % reverse('college:disclosures:school_search')
...
class OfferTest(django.test.TestCase):
    ...
    def test_offer(self):
        ...
        url = reverse('college:disclosures:offer')
        # url_test = ('college:disclosures:offer_test')
...
class APITests(django.test.TestCase):
    ...
    def test_school_json(self):
        ...
        url = reverse('college:disclosures:school-json', args=['155317'])
    ...
    def test_constants_json(self):
        ...
        url = reverse('college:disclosures:constants-json')
        resp = self.client.get(url)
    ...
    def test_national_stats_json(self):
        ...
        url = reverse(
            'college:disclosures:national-stats-json', args=['408039'])
        resp = self.client.get(url)
        self.assertIn(b'retentionRateMedian', resp.content)
        self.assertEqual(resp.status_code, 200)
        url2 = reverse(
            'college:disclosures:national-stats-json', args=['000000'])
    ...
    def test_expense_json(self):
        ...
        url = reverse('college:disclosures:expenses-json')
        resp = self.client.get(url)
        self.assertIn(b'Other', resp.content)

    @mock.patch('college.views.get_json_file')
    ...
    def test_program_json(self):
        ...
        url = reverse('college:disclosures:program-json', args=['408039_981'])
        resp = self.client.get(url)
        self.assertIn(b'housing', resp.content)
        self.assertIn(b'books', resp.content)
        bad_url = reverse('college:disclosures:program-json', args=['408039'])
        resp2 = self.client.get(bad_url)
        self.assertEqual(resp2.status_code, 400)
        self.assertTrue(b'Error' in resp2.content)
        url3 = reverse('college:disclosures:program-json', args=['408039_xyz'])
        resp3 = self.client.get(url3)
        self.assertEqual(resp3.status_code, 400)
        self.assertIn(b'Error', resp3.content)
        url4 = reverse(
            'college:disclosures:program-json', args=['408039_<script>'])
        resp4 = self.client.get(url4)
        self.assertEqual(resp4.status_code, 400)
        self.assertIn(b'Error', resp4.content)
        url5 = reverse('college:disclosures:program-json', args=['x08039_981'])
...
class VerifyViewTest(django.test.TestCase):
    ...
    post_data = {'oid': 'f38283b5b7c939a058889f997949efa566c616c5',
                 ...
                 'errors': 'none',
                 'URL': 'fake-url.com'}
    url = reverse('college:disclosures:verify')
