...
class ProgramSerializer(serializers.Serializer):
    ...
    accreditor = serializers.CharField(allow_blank=True, required=False)
    median_salary = serializers.IntegerField(allow_null=True, required=False)  # 31240
    average_time_to_complete = serializers.IntegerField(  # 36
        allow_null=True, required=False
    )
    completion_rate = serializers.FloatField(allow_null=True, required=False)  # 0.29
    default_rate = serializers.FloatField(allow_null=True, required=False)  # 0.23
    job_placement_rate = serializers.FloatField(allow_null=True, required=False)  # 0.7
    job_placement_note = serializers.CharField(allow_blank=True, required=False)
    ...
    cip_code = serializers.CharField(allow_blank=True, required=False)  # '12.0504'
    completers = serializers.IntegerField(allow_null=True, required=False)
    completion_cohort = serializers.IntegerField(allow_null=True, required=False)
...
def strip_control_chars(ustring):
    return ustring.replace("\x97", "-").replace("\x96", "-")
...
def clean(data):
    ...
    cleaned_data = {
        k: clean_number_as_string(v) for k, v in dict.items(data) if k in number_fields
    }
    cleaned_data.update(
        {
            k: clean_string_as_string(v)
    ...
    cleaned_data["ope_id"] = cleaned_data["ope_id"].replace("-", "").replace(" ", "")
    ...
    for row in raw_data:
        ...
        if serializer.is_valid():
            ...
            if not validate_pid(data["program_code"]):
                print("ERROR: invalid program code: " "{}".format(data["program_code"]))
                continue
