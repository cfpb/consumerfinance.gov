...
class ProgamDataTest(django.test.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_program_data = {
            "earnings": {"median_earnings": 3000},
            "debt": {"median_debt": 20000, "monthly_debt_payment": 400},
            "credential": {"level": "5"},
            "code": "5101",
            "title": "Nursing",
            "counts": {"titleiv": 3000},
        ...
        self.empty_mock_program_data = {
            "earnings": {"median_earnings": None},
            "debt": {"median_debt": None, "montly_debt_payent": None},
        }
    ...
    def test_empty_program_data_fails(self):
        self.assertFalse(
            update_colleges.test_for_program_data(self.empty_mock_program_data)
        )
    ...
    def test_program_creation_success(self):
        ...
        self.assertTrue(
            Program.objects.filter(
                institution=self.school, program_code="5101-5"
            ).exists()
        )
...
class TestScripts(django.test.TestCase):
    ...
    def test_get_grad_level(self):
        ...
        self.assertEqual(process_cohorts.get_grad_level(level_2_school), "2")
        self.assertEqual(process_cohorts.get_grad_level(level_4_school), "4")
        self.assertEqual(process_cohorts.get_grad_level(level_5_school), "4")
    ...
    def test_update_colleges_success_community_college(self, mock_get_data):
        ...
        (NO_DATA, endmsg) = update_colleges.update(single_school=100636)
        self.assertTrue(len(NO_DATA) == 0)
        self.assertTrue("Updated" in endmsg)
    ...
    def test_update_colleges_single_not_operating(self, mock_get_data):
        ...
        (NO_DATA, endmsg) = update_colleges.update(single_school=408039)
        self.assertTrue(len(NO_DATA) == 0)
    ...
    def test_update_colleges_no_data(self, mock_get_data):
        ...
        self.assertEqual(
            ...
            School.objects.filter(operating=True)
            .exclude(pk__in=FAKE_SCHOOL_PKS)
            .count(),
        )
    ...
    @patch(
        "paying_for_college.disclosures.scripts.update_ipeds.download_zip_file"
    )  # noqa
    @patch(
        "paying_for_college.disclosures.scripts.update_ipeds.clean_csv_headings"
    )  # noqa
    ...
    def test_send_stale_notifications(self, mock_mail):
        ...
        msg = notifications.send_stale_notifications(
            add_email=["abc@def.com", "ghi@jkl.com"]
        )
