...
class Export(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def post(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="registrations-%s.csv"' % now.strftime('%Y%m%d%I%M')

        writer = csv.writer(response)
        writer.writerow(['id','company_name', 'address1','address2','city','state','zip', 'tax_id', 'website', 'company_phone',
                        'contact_name','contact_title', 'contact_email', 'contact_phone', 'contact_ext'])

        for row in data:
            writer.writerow([row.id,row.company_name, row.address1, row.address2, row.city, row.state, row.zip, row.tax_id,
                            row.website, row.company_phone, row.contact_name, row.contact_title, 
                            row.contact_email, row.contact_phone, row.contact_ext])
