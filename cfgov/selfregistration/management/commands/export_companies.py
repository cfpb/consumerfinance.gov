...
class Command(BaseCommand):
    ...
    def handle(self, output_dir , **options):
        ...
        writer.writerow(['company_name', 'address1','address2','city','state','zip', 'tax_id', 'website', 'company_phone',
                        'contact_name','contact_title', 'contact_email', 'contact_phone', 'contact_ext'])

        for row in companies_to_export:
            writer.writerow([row.company_name, row.address1, row.address2, row.city, row.state, row.zip, row.tax_id,
