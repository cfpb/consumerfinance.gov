...
class DataConverter(PageDataConverter):
    ...
    def get_agenda_dict(self, doc_agenda):
        ...
        for i in range(len(doc_agenda)):
            ...
            for t in [('beginning_time', 'start_dt'),
                      ('ending_time', 'end_dt')]:
                agenda_dict['agenda_items-' + str(i) + '-value-' + t[1]] = \
                    self.format_agenda_times(doc_agenda, t, i)

            agenda_dict['agenda_items-' + str(i) + '-value-description'] = \
                doc_agenda[i].get('desc', u'')
            agenda_dict['agenda_items-' + str(i) + '-value-location'] = \
                doc_agenda[i].get('location', u'')
            agenda_dict['agenda_items-' + str(i) + '-value-speakers-count'] = \
                len(doc_agenda[i].get('speakers', u''))
            for y in range(len(doc_agenda[i].get('speakers', u''))):
                speakers_prefix = ('agenda_items-'
                                   + str(i)
                                   + '-value-speakers-'
                                   + str(y))
                agenda_dict[speakers_prefix + '-value-name'] = \
                    doc_agenda[i]['speakers'][y].get('name', u'')
                agenda_dict[speakers_prefix + '-value-url'] = \
                    doc_agenda[i]['speakers'][y].get('url', u'')
    ...
    def convert(self, doc):
        ...
        post_dict.update(self.get_times_dict(doc.get('beginning_time'),
                                             doc.get('ending_time')))
        ...
        for tense in ['archive', 'live', 'future']:
            post_dict[tense + '_body'] = self.format_time_period_content(
                doc.get(tense), tense)
