...
# Define a set of 'embed handlers' and 'link handlers'. These handle the
# translation of 'special' HTML elements in rich text - ones which we do not
# want to include verbatim in the DB representation because they embed
# information which is stored elsewhere in the database and is liable to change
# - from real HTML representation to DB representation and back again.


class PageLinkHandler(object):
    """
    Handle <a> elements with an attribute of data-linktype="page".

    The resulting element in the database representation will be:
    ...
    def get_db_attributes(tag):
        """
        Return a dict of link attributes.

        Given an <a> tag that we've identified as a page link embed
        (because it has a data-linktype="page" attribute), return the dict
        of the attributes we should have on the resulting
        <a linktype="page"> element.
        """
    ...
    def expand_db_attributes(attrs, for_editor):
        try:
            ...
            return '<a %shref="%s">' % (
                editor_attrs, escape(page.specific.url))
...
class DbWhitelister(Whitelister):
    """
    Create a whitelisting engine to convert HTML from the rich text editor.

    Content is converted into the pseudo-HTML format stored in the database
    (in which images, documents and other linked objects are identified
    by ID rather than URL):

    * implements a 'construct_whitelister_element_rules' hook so other apps
    can modify the whitelist ruleset (e.g. to permit additional HTML elements
    beyond those in the base Whitelister module);
    * replaces any element with a 'data-embedtype' attribute with an <embed>
    element, with attributes supplied by the handler for that type
    as defined in EMBED_HANDLERS;
    * rewrites the attributes of any <a> element with a 'data-linktype'
    attribute, as determined by the handler for that type defined in
    LINK_HANDLERS, while keeping the element content intact.
    """
...
def extract_attrs(attr_string):
    """
    Method to extract tag attributes as a dict. Does not escape HTML entities!
    """
    attributes = {}
...
class RichText(object):
    ...
    def __html__(self):
        return (
            '<div class="rich-text">' + expand_db_html(self.source) + '</div>'
        )
...
class FeatureRegistry(object):
    """
    A central store of information about optional features that can be enabled
    in rich text editors by passing a ``features`` list to the RichTextField,
    such as how to whitelist / convert HTML tags, and how to enable the feature
    on various editors.

    This information may come from diverse sources - for example, wagtailimages
    might define an 'images' feature and a hallo.js plugin for it, while a
    third-party module might define a TinyMCE plugin for the same feature.
    The information is therefore collected into this registry via
    the 'register_rich_text_features' hook.
    """
    def __init__(self):
        # Has the register_rich_text_features hook been run for this registry?
        self.has_scanned_for_features = False

        # a dict of dicts, one for each editor (hallo.js, TinyMCE etc);
        # each dict maps feature names to 'plugin' objects that define how to
        # implement that feature (e.g. paths to JS files to import).
        # The API of that plugin object is not defined
        # here, and is specific to each editor.
        self.plugins_by_editor = {}

        # a list of feature names that will be applied on rich text areas
        # that do not specify an explicit `feature` list.
        # RemovedInWagtail114Warning: Until Wagtail 1.14, features listed here
        # MUST also update the legacy global halloPlugins list (typically by
        # calling registerHalloPlugin within an insert_editor_js hook).
        # This is because we special-case rich text areas without an explicit
        # `feature` list, to use the legacy halloPlugins list instead of
        # the one constructed using construct_plugins_list; this ensures that
        # any user code that fiddles with halloPlugins will continue to work
        # until Wagtail 1.14.
