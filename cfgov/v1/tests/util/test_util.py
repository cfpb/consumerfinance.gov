...
class TestSecondaryNav(TestCase):
    ...
    def test_nav_for_browse_page_includes_only_its_children(self):
        ...
        self.assertEqual(
            nav[0]["children"][0]["title"], self.child_of_browse_page1.title
        )

        self.assertEqual(nav[1]["title"], self.browse_page2.title)
        self.assertEqual(nav[1]["children"], [])

    def test_nav_for_child_of_browse_page_includes_only_children_of_parent_browse_page(  # noqa: B950
        self,
    ):
        nav, has_children = util.get_secondary_nav_items(
        ...
        self.assertEqual(
            nav[1]["children"][0]["title"], self.child_of_browse_page2.title
        )
