...
class CFGOVPage(Page):
    ...
    def get_appropriate_page_version(self, hostname):
    ...
    def get_breadcrumbs(self, request):
        ...
        for i, ancestor in enumerate(ancestors):
            if ancestor in home_page_children:
                return [ancestor.get_appropriate_page_version(request.site.hostname) for ancestor in ancestors[i+1:]]
        return []
    ...
    def route(self, request, path_components):
        if path_components:
            ...
            page = self.get_appropriate_page_version(request.site.hostname)
            if page:
