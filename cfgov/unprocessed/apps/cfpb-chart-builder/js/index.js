...
class Chart {
   ...
   */
  update(newOptions) {
    const needNewMapShapes =
      this.chartOptions.type === 'geo-map' &&
    ...
    if (typeof newOptions.source === 'undefined') {
      ...
      getData(this.chartOptions.source)
        .then((data) => {
          this.chartOptions.data = data;

          if (needNewMapShapes) {
            this.chartOptions.needNewMapShapes = true;
            fetchMapShapes(this.chartOptions.metadata)
              .then((shapes) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      function afterUpdateHandler() {
        this.highchart.removeEventListener(
          'afterUpdate',
          afterUpdateHandlerBinded,
        );
        resolve();
      }
    });
...
export { createChart, createCharts };
