let linkElement, hidemenu, dropdown, dropping;

function escHandler( e ) {
  if ( e.keyCode == 27 ) {
    window.clearTimeout( countdown );
    const target = $( '#interScreen p:first a' ).attr( 'href' );
    $( '#interScreen' ).remove();
...
function dropMenu() {
  ...
  const target = menuItem.attr( 'href' );
...
$( document ).ready( function() {
  ...
  $( '#header nav a' ).mouseenter( function() {
    ...
    const clicked = $( this );
    const target = clicked.attr( 'href' );
    if ( target == '/' ) {
      return true;
    }
...
$( function() {
  ...
  const searchLabelNd = $( '#search_form label' );
  if ( $( '#search_form #query' ).val() ) searchLabelNd.fadeOut( 50 );
  else searchLabelNd.fadeIn( 50 );


  // placeholder text Behavior on focus and blur
  $( '#search_form #query' ).focus( function() {
    if ( $( this ).val() ) searchLabelNd.fadeOut( 50 );
    else searchLabelNd.fadeIn( 50 );
  } );
