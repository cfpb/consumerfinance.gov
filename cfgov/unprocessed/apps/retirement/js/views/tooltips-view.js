...
function toolTipper( elem ) {
  ...
  const containerRect = document.querySelector( '#claiming-social-security' ).getBoundingClientRect();
  const rect = elem.getBoundingClientRect();
  const newTop = rect.top - containerRect.top + 30 + 'px';

  newLeft = rect.left - containerRect.left  +
            ( $elem.outerWidth() / 2 - $ttc.outerWidth( true ) / 2 );
  $ttc.css( { top: newTop, left: newLeft + 'px' } );
  ...
  if ( newLeft < pagePadding ) {
    elemCenter = rect.left + ( rect.right - rect.left ) / 2;
    $ttc.css( 'left', -pagePadding / 2 );
    innerTip.css(
      'left',
      elemCenter - innerTip.outerWidth() / 2 - ( pagePadding * 2 )
    );
    outerTip.css(
      'left',
