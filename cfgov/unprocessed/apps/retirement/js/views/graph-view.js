...
function getYourEstimates() {
  ...
  $.when(
    ...
    if ( resp.error === '' ) {
      ...
      textlets.currentAge = window.django.gettext( SSData.currentAge );
      textlets.fullRetirementAge = window.django.gettext(
...
function setAgeWithIndicator( indicatorValue ) {
  ...
  textlets.selectedAge = window.django.gettext( selectedAge );
...
function getTranslations() {
  for ( const key in textlets ) {
    if ( {}.hasOwnProperty.call( textlets, key ) ) {
      textlets[key] = window.django.gettext( textlets[key] );
    }
  }
}
