...
class ConferenceRegistrationForm(AbstractFormBlock):
    ...
    codes = blocks.ListBlock(blocks.CharBlock(label='Code'),
                             label='GovDelivery codes')
    sessions = blocks.ListBlock(blocks.CharBlock(label='Session'),
                                label='Sessions attending')
    capacity = atoms.IntegerBlock(
        help_text='Enter an integer that will be the conference attendance '
                  'limit.'
    )
    at_capacity_message = organisms.FullWidthText(
        help_text='Enter a message that will be shown when the event is at '
                  'capacity'
    )
