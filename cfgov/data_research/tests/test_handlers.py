...
class TestConferenceRegistrationHandler(TestCase):
    ...
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.is_at_capacity')
    def test_process_calls_is_at_capacity(self, mock_capacity):
        self.handler.process(True)
        self.assertTrue(mock_capacity.called)

    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.is_at_capacity')
    def test_process_returns_context_with_flag_when_is_at_capacity(
            self, mock_capacity):
    ...
    def test_is_at_capacity_calls_filter_with_generated_query_from_codes(
            self, mock_model, mock_reduce):
    ...
    @mock.patch('data_research.handlers.ConferenceRegistration')
    def test_is_at_capacity_returns_False_for_attendance_less_than_capacity(
            self, mock_model):
        mock_model.objects.filter().count.return_value = 0
    ...
    @mock.patch('data_research.handlers.ConferenceRegistration')
    def test_is_at_capacity_returns_True_for_attendance_at_capacity(
            self, mock_model):
    ...
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.is_at_capacity')
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.get_post_data')
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.get_response')
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationForm')
    def test_process_calls_get_post_data_for_submitted_form(
            self, mock_form, mock_get_response, mock_get_post_data,
            mock_capacity):
    ...
    def test_process_returns_get_response_call_for_submission(
            self, mock_get_response, mock_capacity):
        mock_capacity.return_value = False
        result = self.handler.process(True)
    ...
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.get_sessions')
    def test_get_post_data_sets_sessions_from_form_sessions(
            self, mock_get_sessions):
        mock_get_sessions.return_value = self.handler.block_value['sessions']
        result = self.handler.get_post_data()
        self.assertIn('sessions', result)
        self.assertEqual(json.dumps(self.handler.block_value['sessions']),
                         result['sessions'])
    ...
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.subscribe')
    @mock.patch('data_research.handlers.ConferenceRegistrationHandler.fail')
    def test_get_response_calls_fail_for_failed_subscribe(
            self, mock_fail, mock_subscribe):
        form = mock.Mock()
        mock_subscribe.return_value = False
        self.handler.get_response(form)
        mock_fail.assert_called_with(form)
    ...
    @mock.patch(
        'data_research.handlers.ConferenceRegistrationHandler.subscribe')
    @mock.patch('data_research.handlers.ConferenceRegistrationHandler.fail')
    def test_get_response_calls_success_for_successful_subscription(
            self, mock_fail, mock_subscribe, mock_success):
        form = mock.Mock()
    ...
    def test_subscribe_instantiates_GovDelivery_with_account_code(
            self, mock_gd, mock_messages, mock_settings):
    ...
    @mock.patch('data_research.handlers.messages')
    @mock.patch('data_research.handlers.GovDelivery')
    def test_subscribe_sets_subscriptions(
            self, mock_gd, mock_messages, mock_settings):
        self.handler.subscribe('em@il.com', ['code'])
    ...
    @mock.patch('data_research.handlers.messages')
    @mock.patch('data_research.handlers.GovDelivery')
    def test_subscribe_returns_True_for_success(
            self, mock_gd, mock_messages, mock_settings):
    ...
    @mock.patch('data_research.handlers.messages')
    @mock.patch('data_research.handlers.GovDelivery')
    def test_subscribe_returns_False_for_nonset_account_code(
            self, mock_gd, mock_messages, mock_settings):
    ...
    @mock.patch('data_research.handlers.messages')
    @mock.patch('data_research.handlers.GovDelivery')
    def test_subscribe_returns_False_when_setting_subscriptions(
            self, mock_gd, mock_messages, mock_settings):
    ...
    def test_get_sessions_returns_strings_from_block_value_if_in_post_data(
            self):
    ...
    @mock.patch('data_research.handlers.JsonResponse')
    @mock.patch('data_research.handlers.messages')
    def test_fail_calls_returns_JsonResponse_with_nonajax_request(
            self, mock_messages, mock_json_response):
        form = mock.Mock()
        form.errors = {'err': ['error message']}
        self.handler.fail(form)
        mock_messages.error.assert_called_with(self.handler.request,
                                               form.errors['err'][0])
