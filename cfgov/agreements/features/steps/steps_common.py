...
# Prototype: I select what I call the "card issuer" drop list and search and select "bank of america"
@when(u'I select what I call the "{element_nickname}" drop list and search "{entered_text}" and choose first')
def step(context, element_nickname, entered_text):
    context.website.search_and_select_first_from_drop_list(element_nickname, entered_text)

# Prototype: I select what I call the "search" text box and search "reverse" and choose "What is a reverse mortgage?"
@when(u'I select what I call the "{element_nickname}" text box and search "{entered_text}" and choose "{link_text}"')
def step(context, element_nickname, entered_text, link_text):
    context.website.search_and_select_link_from_autocomplete(element_nickname, entered_text, link_text)


# Then statements
# these are the assertion(s) that verify/validate that the test did or did not pass
...
def step(context):
    assert_that(context.website.is_page_properly_loaded(), equal_to(True), 'Page did not properly load.')

@then('I should see the page title as "{key_text} > Consumer Financial Protection Bureau"')
...
def step(context, key_text):
    ...
    assert_that(context.website.is_text_last_in_breadcrumb(key_text), equal_to(True), error_message)

@then(u'I should find the text "{expected_text}" on the page')
def step(context, expected_text):
    error_message = 'Text "%s" was not found on the page" % key_text'
    assert_that(context.website.is_text_in_body(expected_text), equal_to(True), error_message)
